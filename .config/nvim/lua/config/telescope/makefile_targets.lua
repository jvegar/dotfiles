...
local function run_make_command(target)
  ...
  if current_make_buf and vim.api.nvim_buf_is_valid(current_make_buf) then
    ...
    -- Clear new buffer and window if none exists
    ...
    vim.api.nvim_set_lines(current_make_buf, 0, -1, false, { "Press ENTER to close this buffer", "" })
  end
  ...
  -- Set buffer variable for job ID
