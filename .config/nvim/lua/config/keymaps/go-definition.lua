-- Store the original tab and list of spawned tabs
...
local function go_to_definition_tab()
  ...
  vim.lsp.buf_request(0, "textDocument/definition", params, function(err, result)
    ...
    vim.notify("No valid location found for definition.")
    vim.cmd("tabclose")
  end)
end
...
local function close_tabs_and_return()
  ...
  -- Return to the original tab if it's still valid
...
vim.keymap.set("n", "gd", go_to_definition_tab, { noremap = true, silent = true, desc = "Go to definition in new tab" })

-- Keymap to close all spawned tabs and return to the original tab
vim.keymap.set("n", "<leader>q", close_tabs_and_return, { noremap = true, silent = true, desc = "Close all spawned tabs and return" })
